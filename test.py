...
from DeviceManager import *
from database import PrintLoger
import time

logger = PrintLoger()
dv = DeviceManager(logger)
dv.device_name = 'ASRL3::INSTR'
dv.connect()

counter = 0
counter_h = 0

# A properly formatted hex string (no commas, just hex)
packet = "DE280AFF34005411010A010A0AFF34005411010A010A0AFF34005411010A010A"
...
while True:
    ...
    # Format counters as hex with two digits
    ch_hex = f"{counter_h:02X}"
    c_hex = f"{counter:02X}"

    # Build the final hex string
    temp_packet = f"{c_hex}{ch_hex}{packet}{c_hex}{ch_hex}"

    # Increment counters
    counter += 1

    # Write the hex string to the device
    dv.write(temp_packet)
    time.sleep(0.005)
